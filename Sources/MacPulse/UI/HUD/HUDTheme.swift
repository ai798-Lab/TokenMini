...
enum HUD {
    ...
    static var faint: Color { prism ? Prism.faint : Color(red: 0.47, green: 0.56, blue: 0.67) }

    // 线
    static var hairline: Color { cyan.opacity(0.16) }
    static var gridline: Color { prism ? Prism.line.opacity(0.55) : Color.white.opacity(0.05) }

    static func mono(_ size: CGFloat, _ weight: Font.Weight = .regular) -> Font {
        .system(size: prism ? max(10, size) : size, weight: weight, design: prism ? .default : .monospaced)
    }
...
struct HUDGauge: View {
        ...
        if settings.isPrism {
            GeometryReader { proxy in
                ZStack(alignment: .leading) {
                    RoundedRectangle(cornerRadius: 2).fill(Prism.line.opacity(0.6))
    ...
    private var hudGauge: some View {
        HStack(spacing: 2) {
...
struct HUDStatusDot: View {
    ...
    var body: some View {
        Rectangle()
            ...
            .onAppear {
                guard pulses && !reduceMotion && !settings.isPrism else { return }
...
struct HUDChip: View {
    ...
    var body: some View {
        Text(text)
            .font(HUD.mono(settings.isPrism ? 10 : 8, .semibold))
            .foregroundStyle(color)
            .padding(.horizontal, settings.isPrism ? 6 : 5)
            .padding(.vertical, settings.isPrism ? 3 : 1.5)
            .background {
                if settings.isPrism {
                    RoundedRectangle(cornerRadius: Prism.controlRadius).fill(color.opacity(0.10))
                        .overlay(RoundedRectangle(cornerRadius: Prism.controlRadius)
                            .strokeBorder(color.opacity(0.3), lineWidth: 1))
                } else {
                    CutCorner(cut: 3).fill(color.opacity(0.12))
                        .overlay(CutCorner(cut: 3).stroke(color.opacity(0.45), lineWidth: 1))
                }
            }
            .lineLimit(1)
    }
...
struct HUDButtonStyle: ButtonStyle {
    ...
    private struct Chrome: View {
        ...
        var body: some View {
            if settings.isPrism {
                configuration.label.modifier(PrismControlChrome(active: filled, accent: accent,
